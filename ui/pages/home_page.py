...
class HomePage(QWidget):
    ...
    def _init_resolution_controls(self):
        ...
        for text, callback in (("16:9", lambda: self.set_aspect_ratio(16, 9)), ("21:9", lambda: self.set_aspect_ratio(21, 9)), ("Custom", self.toggle_custom_res_inputs)):
            b = QPushButton(text); b.setProperty("class", "resBtn"); b.clicked.connect(callback); row.addWidget(b)
        self.res_custom_container = QWidget(); self.res_custom_container.hide()
        rc = QHBoxLayout(self.res_custom_container); rc.setContentsMargins(4, 0, 0, 0)
        self.res_w_input = QLineEdit("1000"); self.res_h_input = QLineEdit("562")
        for inp in (self.res_w_input, self.res_h_input):
            inp.setFixedWidth(70); inp.setValidator(QIntValidator(100, 5000)); inp.editingFinished.connect(self.apply_custom_res)
        rc.addWidget(QLabel("W")); rc.addWidget(self.res_w_input); rc.addWidget(QLabel("H")); rc.addWidget(self.res_h_input)
        row.addWidget(self.res_custom_container); row.addStretch()
        btn_fs = QPushButton("Fullscreen"); btn_fs.setProperty("class", "resBtn"); btn_fs.clicked.connect(self.toggle_fullscreen); row.addWidget(btn_fs)
        self.col_prev_layout.addLayout(row)

    def _init_menu_panel(self):
        self.menu_frame = QFrame(); self.menu_frame.setFixedWidth(220); self.menu_frame.setObjectName("menu_frame")
        self.menu_layout = QVBoxLayout(self.menu_frame); self.menu_layout.setAlignment(Qt.AlignTop); self.menu_layout.setSpacing(7); self.menu_layout.setContentsMargins(14, 14, 14, 14)
        ...
        images = self.add_menu_group("Images"); self.add_sub_item(images, "Frame Image", "frame_image"); self.add_sub_item(images, "NTP Image", "ntp_image")
        self.menu_layout.addStretch()

    def _init_control_panel(self):
        self.ctrl_frame = QFrame(); self.ctrl_frame.setFixedWidth(330); self.ctrl_frame.setObjectName("ctrl_frame")
        ctrl = QVBoxLayout(self.ctrl_frame); ctrl.setContentsMargins(18, 14, 18, 18); self.stack = QStackedWidget()
        ...
        row = QHBoxLayout(); self.color_preview_box = QLabel(); self.color_preview_box.setFixedSize(58,58); self.lbl_color_name = QLabel("Red"); self.lbl_color_name.setProperty("class", "mutedLabel")
        self.hex_input = QLineEdit("#CC0000FF"); self.hex_input.setMaxLength(9); self.hex_input.textChanged.connect(self.hex_changed)
        info = QVBoxLayout(); info.addWidget(self.lbl_color_name); info.addWidget(self.hex_input); row.addWidget(self.color_preview_box); row.addLayout(info,1)
        btn_pick = QPushButton("Pick"); btn_pick.clicked.connect(self.open_color_dialog); row.addWidget(btn_pick); lc.addLayout(row); lc.addSpacing(12)
        ...
        image_page = QWidget(); li = QVBoxLayout(image_page); li.setContentsMargins(0,0,0,0); li.setAlignment(Qt.AlignTop); li.addWidget(QLabel("IMAGE", objectName="sectionHeader"))
        self.mini_preview = QLabel("Drop or select an image"); self.mini_preview.setFixedHeight(150); self.mini_preview.setAlignment(Qt.AlignCenter); self.mini_preview.setObjectName("imageDropPreview"); li.addWidget(self.mini_preview)
        img_row = QHBoxLayout(); up = QPushButton("Select Image"); up.clicked.connect(self.upload_img); clr = QPushButton("Remove"); clr.clicked.connect(self.remove_img); img_row.addWidget(up); img_row.addWidget(clr); li.addLayout(img_row); li.addSpacing(10)
        self.sl_scale, _ = self.make_smart_row("Scale", li, 100, 10, 300); self.sl_x, _ = self.make_smart_row("X", li, 0, -5000, 5000); self.sl_y, _ = self.make_smart_row("Y", li, 0, -5000, 5000)
        self.stack.addWidget(color_page); self.stack.addWidget(image_page); ctrl.addWidget(self.stack, 1)
    ...
    def frame_image_hit_test(self, point):
        return point.y() < 135

    def set_mode(self, mode):
        self.current_base_mode=mode
        real = {"frame":"frame_incognito","inactive_tab":"inactive_tab_incognito","frame_image":"frame_image_incognito","omnibox_background":"omnibox_background_incognito","omnibox_text":"omnibox_text_incognito"}.get(mode,mode) if self.chk_incognito.isChecked() else mode
        self.current_edit_mode=real
        if "image" in real:
            self.stack.setCurrentIndex(1); path=self.theme_data.get(real)
            if path and os.path.exists(path):
                pix=QPixmap(path); self.mini_preview.setPixmap(pix.scaled(self.mini_preview.size(),Qt.KeepAspectRatio,Qt.SmoothTransformation))
            else: self.mini_preview.setPixmap(QPixmap()); self.mini_preview.setText("Drop or select an image")
            self.load_image_params(real)
        else:
            self.stack.setCurrentIndex(0); c=self.color_from_rgba_hex(self.theme_data.get(real,"#FFFFFFFF")); self.block_signals(True); self.hue_slider.setValue(max(0,c.hsvHue())); self.sl_r.setValue(c.red()); self.sl_g.setValue(c.green()); self.sl_b.setValue(c.blue()); self.sl_a.setValue(c.alpha()); self.update_color_info(c); self.block_signals(False)
    ...
    def load_image_params(self, mode):
        ...
        if props:
            self.block_signals(True); self.sl_scale.setValue(int(props.get("scale",100))); self.sl_x.setValue(int(props.get("x",0))); self.sl_y.setValue(int(props.get("y",0))); self.block_signals(False); return
    ...
    def load_image_from_path(self, path):
        ...
        if self.p_settings.get_resize_large() and (pix.width()>4096 or pix.height()>4096): pix=pix.scaled(4096,4096,Qt.KeepAspectRatio,Qt.SmoothTransformation); pix.save(path,"PNG") if False else None
        self.theme_data[self.current_edit_mode]=path; self.theme_data.pop(self.current_edit_mode+"_properties",None); self.renderer.invalidate_image_cache(path)
        self.mini_preview.setPixmap(pix.scaled(self.mini_preview.size(),Qt.KeepAspectRatio,Qt.SmoothTransformation)); self.load_image_params(self.current_edit_mode); self.renderer.apply_image(self.current_edit_mode)
    ...
    def set_aspect_ratio(self,w,h): self.res_custom_container.hide(); self._resize_canvas_and_overlays(1000,max(100,int(1000*h/w))); self.res_w_input.setText("1000"); self.res_h_input.setText(str(self.canvas.height()))
    def apply_custom_res(self):
        try: self._resize_canvas_and_overlays(int(self.res_w_input.text()),int(self.res_h_input.text()))
    ...
    def hue_changed(self):
        c=QColor(self.sl_r.value(),self.sl_g.value(),self.sl_b.value()); h=self.hue_slider.value(); s=max(0,c.hsvSaturation()) or 255; v=c.value(); nc=QColor.fromHsv(h,s,v); self.block_signals(True); self.sl_r.setValue(nc.red()); self.sl_g.setValue(nc.green()); self.sl_b.setValue(nc.blue()); self.block_signals(False); self.slider_color_changed()
    ...
    def upload_img(self):
        f,_=QFileDialog.getOpenFileName(self,"Select Image",self.p_settings.get_last_import_dir(),"Images (*.png *.jpg *.jpeg *.webp)"); self.load_image_from_path(f) if f else None
    def remove_img(self):
    ...
    def make_smart_row(self,label,layout,val=0,min_v=0,max_v=255):
        ...
        sl.sliderReleased.connect(self.slider_released); sl.valueChanged.connect(lambda v:inp.setText(str(v))); inp.editingFinished.connect(lambda:sl.setValue(max(min_v,min(max_v,int(inp.text() or val))))); btn_l.clicked.connect(lambda:sl.setValue(sl.value()-1)); btn_r.clicked.connect(lambda:sl.setValue(sl.value()+1))
        row.addWidget(lbl); row.addWidget(btn_l); row.addWidget(sl,1); row.addWidget(btn_r); row.addWidget(inp); layout.addLayout(row); return sl,inp
